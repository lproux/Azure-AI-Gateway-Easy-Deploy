...
def autodiscover_api_id():
    ...
    try:
        ...
        return None
# ENHANCED: Better validation and diagnostics
...
if not API_ID:
    ...
    if discovered_api_id:
        ...
        # ADDED: Verify the discovered API exists
        print(f"[*] Verifying API exists...")
        try:
            subscription_id_local = globals().get("subscription_id") or ENV.get("SUBSCRIPTION_ID")
            if subscription_id_local:
                verify_url = (f"https://management.azure.com/subscriptions/{subscription_id_local}"
                             f"/resourceGroups/{RESOURCE_GROUP}/providers/Microsoft.ApiManagement"
                             f"/service/{APIM_SERVICE}/apis/{API_ID}?api-version=2022-08-01")
                verify_result = subprocess.run([az_cli, "rest", "--method", "get", "--url", verify_url],
                                             capture_output=True, text=True, timeout=30)
                if verify_result.returncode == 0:
                    print(f"[OK] API verified: {API_ID}")
                else:
                    print(f"[WARN] Could not verify API: {API_ID}")
        except Exception:
            pass
    else:
        # Fallback to default
        ...
        os.environ["APIM_API_ID"] = API_ID
        print(f"[!] Could not auto-discover API_ID")
        print(f"[!] Using default: {API_ID}")
        print(f"[INFO] To manually set: os.environ['APIM_API_ID'] = 'your-api-id'")
else:
